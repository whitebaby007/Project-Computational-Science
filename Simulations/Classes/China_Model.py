...
class Model:
    def __init__(self, width=90, height=60, nHuman=300, nMovehuman=300, initHumanInfected=0.01, initMovehumanInfected=0.01,
                 humanInfectionProb=0.75, VaccinationRate_PerUpdate = 0.0, InfectionProb_Vaccinated = 0.25, deathrate=0.05, immune=0.5,  image_path='China_Population.png'):

        # Process the image and create a binary grid
        original_image = Image.open('China_Population.png')
        resized_image = original_image.copy().resize((90, 60))
        gray_image = resized_image.convert('L')
        binary_image = gray_image.point(lambda x: 0 if x < 220 else 1, '1')
